...
| `mode` | `--mode <value>` |
| `retrieve_url` | `--retrieve-url <url>` |
| `retrieve_mode` | `--retrieve-mode <value>` |
| `sidecar_url` | `--sidecar-url <url>` |
| `top_k` | `--top-k <n>` |
  ...
  "items": [
    {
      ...
      "retrieval": {
        ...
        "triples_count": 20,
        "chunk_count": 3,
        "debug_strategies": ["path1_path2_merge", "chunk_lookup"]
      },
      "error": ""
    }
  ]
...
`retrieval.context_source` is important for interpreting scores:

- `service_retrieve`: each question first called the Go service
...
- `corpus_keyword_overlap`: the worker selected chunks directly from the raw
  ...
  retrieval quality.
- `none`: no context was available.

When `retrieve_url` is set, retrieval failure for one question should mark that
item failed; the worker must not silently fall back to `corpus_keyword_overlap`.
