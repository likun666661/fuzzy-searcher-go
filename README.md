...
| `rerank-merge` | Current main path: fetch path1 raw candidates, rerank them through sidecar, fetch path2 primitive, and merge in Go. |
...
Current Phase 8 expected status with `--mode rerank-merge`: `loader`,
`chunk`, `triple`, and `full` all pass against `demo.json`; the debug strategy
should be `path1_rerank_path2_primitive_merge`.
