...
SHARDS="${PAPER_BENCHMARK_SHARDS:-1}"
SHARD_PROGRESS_POLL="${PAPER_BENCHMARK_SHARD_PROGRESS_POLL:-10}"
...
RUNNER_SCRIPT="scripts/paper_benchmark_worker.py"
SHARD_ARGS=""
if [ "$SHARDS" != "1" ]; then
  RUNNER_SCRIPT="scripts/paper_benchmark_shard_runner.py"
  SHARD_ARGS="--shard-count $SHARDS --progress-poll-seconds $SHARD_PROGRESS_POLL"
fi
